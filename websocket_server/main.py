...
async def main():
    ...
    # connect to mysql
    server1_pool = aiomysql.create_pool(
        host="server1", # MySQL 服务器1地址
        user="root",
        password="password",
        db="testdb",
        minsize=1,
    ...
    server2_pool = aiomysql.create_pool(
        host="server2",
        user="root",
        password="password",
        db="testdb",
    ...
    server3_pool = aiomysql.create_pool(
        host="server3",
        user="root",
        password="password",
        db="testdb",
    ...
    mysql_conn_pools = [server1_pool, server2_pool, server3_pool]

    async def handler(websocket, path):
        await request_handeler(websocket, path, redis, mysql_conn_pool)
